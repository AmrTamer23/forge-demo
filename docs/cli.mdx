...
If you want to use the core API programmatically, see the [#programmatic-usage](cli.mdx#programmatic-usage "mention") section below.
...
These commands help you get started with Forge. If you're just getting started with Electron Forge, we recommend you follow the [.](./ "mention") or [import-existing-project.mdx](import-existing-project.mdx "mention") guides.
...
:::info
For more conceptual details, see the [build-lifecycle.mdx](core-concepts/build-lifecycle.mdx "mention") guide.
:::
...
```bash
  ...
  npm run publish -- --from-dry-run # If there is no publish script: npx electron-forge
...
Forge plugins can override this command to run custom development logic. For example, the [webpack.mdx](config/plugins/webpack.mdx "mention") runs a webpack-dev-server instance to provide live reloading and HMR.
